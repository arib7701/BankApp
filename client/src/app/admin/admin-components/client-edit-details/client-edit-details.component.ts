...
export class ClientEditDetailsComponent implements OnInit, OnDestroy {
  id: number;
  client: Client;
  ...
  onSubmit() {
    ...
    .subscribe(result => {
        ...
        this.flashMessageService.show('Client Information updated successfully.', {cssClass: 'alert-success', timeout: 4000});

    }, error => {
      console.log('Error update client ', error);

      this.flashMessageService.show('Error updating the information. Please try again later.', {cssClass: 'alert-success', timeout: 4000});
    });
  }
